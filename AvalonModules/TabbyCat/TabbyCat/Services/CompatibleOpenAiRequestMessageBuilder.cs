...
public sealed class CompatibleOpenAiRequestMessageBuilder : AiRequesMessageBuilderBase
{
    public override MessageSessionBase Build(AiApiModelBase aiModel)
    {
        var message = new CompatibleRequestModel
        {
        ...
        else if (aiModel.Provider == AiModelType.Custom) message.Model = ((IDeployName)aiModel).DeployName;
        return message;
    }
}