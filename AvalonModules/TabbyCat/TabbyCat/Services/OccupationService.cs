...
public sealed class OccupationService(IUser user, ICustomAssistantOccupationService customAssistantOccupationService)
    : IOccupationService
{
    public async Task<IEnumerable<OccupationType>> GetAllOccupationsAsync()
    {
        var customOccupations =
            await customAssistantOccupationService.QueryAsync(x => x.Email == user.Email && !x.IsDeleted);
        ...
        if (user.LoginSuccess())
            ...
            temps.Add(new OccupationType(AssistantOccupation.Common,
                LocalizationResourceManager.Instance[AssistantOccupation.Common.ToString()]));
        return temps;
    }
}