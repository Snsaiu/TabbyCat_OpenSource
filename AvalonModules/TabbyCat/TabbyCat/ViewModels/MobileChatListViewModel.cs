    ...
    IAiChatMessageRecordService aiChatMessageRecordService) : ViewModelBase
{
    ...
    async partial void OnSelectedChatChanged(AiChatSessionEntity? value)
    {
        ...
        var defaultSessions =
            await aiChatSessionService.QueryAsync(x => x.IsDefault && x.Email == CurrentUser.Email);
    ...
    {
        this.ChatList.Reset((await aiChatSessionService.QueryAsync(x => x.Email == CurrentUser.Email)).OrderByDescending(x=>x.UpdateTime));
